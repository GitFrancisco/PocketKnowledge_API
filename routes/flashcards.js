...
router.post("/create", async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: "Flashcard adicionado com sucesso!", id: insertId  }); 
  } catch (err) {
...
router.get("/find/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const conn = await pool.getConnection();
    const flashcard = await conn.query("SELECT * FROM flashcards WHERE id = ?", [id]);
    conn.release();
...
router.post("/favorite", verifyToken, async (req, res) => {
  ...
  try {
    ...
    conn.release();
    res.status(201).json({ message: "Flashcard adicionado aos favoritos com sucesso!" });

  } catch (err) {
    console.error("Erro ao adicionar flashcard:", err);
    res.status(500).json({ error: "Erro ao adicionar flashcard aos favoritos." });
  }
});
...
router.delete("/removeFavorite/:id", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json({ error: "Flashcard não encontrado nos favoritos." });
    }

    res.status(200).json({ message: "Flashcard removido dos favoritos com sucesso!" });
  } catch (err) {
...
router.get("/listFavoriteFlashcards", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (!flashcards || flashcards.length === 0) {
      return res.status(404).json({ message: "Nenhum flashcard favorito encontrado." });
    }
